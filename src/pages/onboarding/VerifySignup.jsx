...
const VerifySignup = () => {
  ...
  // Fetch the email from the URL params
  useEffect(() => {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const obj = { email: otpEmail, code: getOtpValue() };
      ...
      if (response?.status === 200) {
        ...
        // Redirect to /auth/user-info after successful OTP validation
        navigate("/auth/user-info"); // Change this line to navigate to user info
      }
    } catch (err) {
      ErrorToast(err?.response?.data?.message || "Error validating OTP");
      setLoading(false);
    }
  };
